...
directory_to_write = '/Users/user/Desktop/faces_plus30/'
    ...
    while True:
        ...
        if not results is None:
            ...
            for b in total_boxes:
                ...
                # Рисуем на draw прямоугольник вокруг лица
                #cv2.rectangle(draw, (2*int(b[0]), 2*int(b[1])), (2*int(b[2]), 2*int(b[3])), (0, 255, 0), 3)

                # Рисуем на draw точку в центре лица
                #cv2.line(draw, ((int(b[0]) + int(b[2])), (int(b[1]) + int(b[3]))), 
                #                ((int(b[0]) + int(b[2])), (int(b[1]) + int(b[3]))), (255, 0, 0), 3)
                ...
                if (np.abs(center[1] - 140) < 5):
                    ...
                    face = frame[2*int(b[1])-30:2*int(b[3])+30, 2*int(b[0])-30:2*int(b[2])+30]
